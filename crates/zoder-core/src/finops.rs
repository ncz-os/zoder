...
fn render_finops_text(rep: &FinOpsReport, p: &Paint) -> String {
    ...
    s.push('\n');
    s.push_str(&format!("  total calls:     {}\n", rep.total_calls));
    s.push_str(&format!("  total tokens:    {}\n", rep.total_tokens));
    s.push_str(&format!(
        "  total cost:      {}\n",
        p.warn(&format!("${:.4}", rep.total_cost_usd))
    ));
    s.push('\n');
    ...
    if !rep.by_model_realized.is_empty() {
        ...
        for r in rep.by_model_realized.iter().take(10) {
            let rate = match r.realized_usd_per_mtok {
                Some(v) => p.accent(&format!("${v:.4}/Mtok")),
                None => p.dim("n/a"),
            };
            s.push_str(&format!(
                "    {:<48} {}  {} calls  {}\n",
                r.model,
                p.warn(&format!("${:>10.4}", r.cost_usd)),
                r.calls,
                rate
            ));
        }
    ...
    if !cache_hits.is_empty() {
        ...
        for c in cache_hits.iter().take(10) {
            s.push_str(&format!(
                ...
                c.model,
                p.accent(&format!("${:.4}", c.est_savings_usd)),
                c.est_cached_tokens
            ));
        }
        s.push('\n');
    ...
    s.push('\n');
    s.push_str(&format!(
        "    avg daily {}  trend ${:.6}/day  7d {}  30d {}\n",
        p.warn(&format!("${:.4}", rep.forecast.avg_daily_cost_usd)),
        rep.forecast.trend_usd_per_day,
        p.warn(&format!("${:.4}", rep.forecast.forecast_7d_usd)),
        p.warn(&format!("${:.4}", rep.forecast.forecast_30d_usd))
    ));
    s
}
...
pub fn cli_run(
    ...
    if sub == "forecast" {
        ...
        if json {
            ...
            println!("  avg daily cost:    ${:.4}", fc.avg_daily_cost_usd);
            println!("  median daily cost: ${:.4}", fc.median_daily_cost_usd);
            println!("  trend ($/day):     ${:.6}", fc.trend_usd_per_day);
            println!("  forecast +7d:      ${:.4}", fc.forecast_7d_usd);
            println!("  forecast +30d:     ${:.4}", fc.forecast_30d_usd);
        }
        return Ok(0);
    }
    ...
    match sub {
        "advisor" => {
            ...
            for row in &rep.advisor {
                ...
                println!(
                    "  {}: paid ${:.4} across {} calls; cheapest alt {} @ ${:.4}/Mtok would have cost ${:.4} (save ${:.4}, {:.1}%).",
                    row.paid_model,
                    row.paid_cost_usd,
                    row.calls,
