//! `zoder mcp-server` — Model Context Protocol server exposing
//! `zoder_route` as a real, functional tool over stdio.
//!
//! Usage: `zoder mcp-server` (the `mcp-server` subcommand of the zoder CLI)
//! or `zoder-mcp-server` (standalone binary).  The binary speaks the
