...
enum Cmd {
    ...
    /// runs`, `sops/decide`, or `sops/graph` method on the local zeroclaw
    /// engine, and print a clean result (table for list, plain status for
    /// run / status, confirmation for decide, step graph for graph).
    Sop {
        #[command(subcommand)]
        action: SopCmd,
    },
...
enum SopCmd {
    ...
    /// payload. Read-only: this never mutates the run.
    Graph {
...
async fn cmd_sop(_cli: &Cli, action: &SopCmd) -> anyhow::Result<()> {
    // `graph` is the read-only monitor half of the family and lives in
    // `sop.rs` (it renders a `SopGraphReport` rather than a raw `sops/*`
    // response), so it is dispatched there instead of through the shared
    // `cmd_sop_with_socket` RPC path below.
    if let SopCmd::Graph {
        run_id,
        json,
        no_overlay,
    ...
    {
        return sop::cmd_sop_graph(run_id, *json, !*no_overlay).await;
    }
    let socket = engine_socket_path();
...
async fn cmd_sop_with_socket<W: std::io::Write>(
    ...
    match action {
        ...
        // `graph` never reaches this RPC path: `cmd_sop` intercepts it and
        // dispatches to `sop::cmd_sop_graph`, which speaks `sops/graph` +
        // `sops/run-overlay` and owns its own rendering.
        SopCmd::Graph { .. } => {
            unreachable!("`sop graph` is dispatched by cmd_sop before this point")
        }
    }
}
